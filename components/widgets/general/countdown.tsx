...
export function Countdown({ id, data, setData }: CountdownProps) {
  ...
  const [eventName, setEventName] = useState(
    data?.eventName || "Countdown Event"
  );
    ...
    <Card className="w-full max-w-md mx-auto bg-gradient-to-br from-gray-100 to-white dark:from-gray-900 dark:to-black">
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-4 gap-4 text-center mb-6">
          {Object.entries(timeLeft).map(([unit, value]) => (
            <div
              key={unit}
              className="flex flex-col bg-white dark:bg-gray-800 rounded-lg p-2 shadow-md"
            >
      ...
      <CardFooter>
        <Button
          variant="outline"
          size="sm"
          className="no-drag w-full bg-gray-800 text-white hover:bg-gray-700 dark:bg-gray-200 dark:text-gray-800 dark:hover:bg-gray-300"
          onClick={(e) => {
            e.stopPropagation(); // Prevent grid drag
            setIsEditing(true);
          }}
        >
